(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),t.credentials=e.crossOrigin===`use-credentials`?`include`:e.crossOrigin===`anonymous`?`omit`:`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();function e(){return`
  ...
  `}function t(){let t=document.getElementById(`app`);if(!t)throw Error(`Missing #app root`);t.innerHTML=e()}function n(e){let t=new Set([`ganado`,`perdido`,`pendiente`,`void`,`revision_pendiente`]);return e.filter(e=>t.has(e.estado))}var r=[{text:`Será como árbol plantado junto a corrientes de aguas, que da su fruto en su tiempo y su hoja no cae; y todo lo que hace, prosperará.`,reference:`Salmo 1:3`,focus:`Prosperidad y Fruto`},{text:`Sea la gracia del Señor nuestro Dios sobre nosotros, y confirma sobre nosotros la obra de nuestras manos; sí, la obra de nuestras manos confirma.`,reference:`Salmo 90:17`,focus:`Bendición del Trabajo`},{text:`Encomienda a Jehová tu camino, confía en él; y él hará.`,reference:`Salmo 37:5`,focus:`Dirección y Confianza`},{text:`Jehová guardará tu salida y tu entrada desde ahora y para siempre.`,reference:`Salmo 121:8`,focus:`Protección Total`},{text:`Pon en manos del Señor todas tus obras, y tus proyectos se cumplirán.`,reference:`Proverbios 16:3`,focus:`Éxito en Proyectos`},{text:`Jehová es mi pastor; nada me faltará.`,reference:`Salmo 23:1`,focus:`Paz y Provisión`},{text:`Mira que te mando que te esfuerces y seas valiente; no temas ni desmayes, porque Jehová tu Dios estará contigo dondequiera que vayas.`,reference:`Josué 1:9`,focus:`Fuerza y Victoria`}];function i(e=`daily-verse-container`){let t=document.getElementById(e);if(!t)return;let n=new Date,i=Math.floor((n.getTime()-new Date(n.getFullYear(),0,0).getTime())/864e5)%r.length;function a(e){let n=r[e];t.innerHTML=`
    ...
    `,document.getElementById(`btn-next-verse`)?.addEventListener(`click`,()=>{i=(i+1)%r.length;let e=document.getElementById(`verse-banner-box`);e?(e.classList.add(`verse-fade-out`),setTimeout(()=>{a(i)},150)):a(i)}),document.getElementById(`btn-copy-verse`)?.addEventListener(`click`,e=>{let t=`«${n.text}» — ${n.reference}`;navigator.clipboard.writeText(t).then(()=>{let t=e.currentTarget;t&&(t.innerHTML=`✅`,setTimeout(()=>{t.innerHTML=`📋`},1800))})})}a(i)}function a(e){let t=e.filter(e=>e.estado===`ganado`).length,n=e.filter(e=>e.estado===`perdido`).length,r=e.filter(e=>![`ganado`,`perdido`,`void`].includes(e.estado)).length,i=e.reduce((e,t)=>t.estado===`ganado`?e+Number(t.cuota)-1:t.estado===`perdido`?e-1:e,0),a=t+n;return{wins:t,losses:n,pending:r,units:Number(i.toFixed(2)),roi:a?Number((i/a*100).toFixed(1)):0}}function o(e){return{pendiente:`Pendiente`,ganado:`Ganado`,perdido:`Perdido`,void:`Nulo`,revision_pendiente:`En revisión`}[e]}function s(e,t){let n=e?.trim(),r=t?.trim();return n&&r?{slot:n,client:r}:null}function c(e,t){if(!t)return;if(!document.querySelector(`script[data-rey-taco-adsense]`)){let e=document.createElement(`script`);e.async=!0,e.crossOrigin=`anonymous`,e.dataset.reyTacoAdsense=`true`,e.src=`https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=${encodeURIComponent(t.client)}`,document.head.append(e)}e.classList.remove(`hidden`),e.innerHTML=`<span class="ad-label">PUBLICIDAD</span>`;let n=document.createElement(`ins`);n.className=`adsbygoogle`,n.style.display=`block`,n.dataset.adClient=t.client,n.dataset.adSlot=t.slot,n.dataset.adFormat=`auto`,n.dataset.fullWidthResponsive=`true`,e.append(n);try{let e=window;(e.adsbygoogle??=[]).push({})}catch{e.classList.add(`hidden`)}}var l=new Set;function u(e){if(l.has(e))return;l.add(e);let t=window;(t.dataLayer??=[]).push({event:e})}function d(e,t){if(!e||typeof IntersectionObserver>`u`)return;let n=new IntersectionObserver(e=>{e.some(e=>e.isIntersecting)&&(u(t),n.disconnect())},{threshold:.25});n.observe(e)}function f(e){let t=document.createElement(`div`);return t.textContent=String(e??``),t.innerHTML}function p(e){let t=[`pendiente`,`ganado`,`perdido`,`void`,`revision_pendiente`],n=String(e.estado??`pendiente`);return{id:e.id??crypto.randomUUID(),categoria:String(e.categoria??`Deportes`),partido:String(e.partido??`Evento por confirmar`),pick:String(e.pick??`Selección por confirmar`),cuota:e.cuota??`—`,confianza:e.confianza??`—`,razonamiento:String(e.razonamiento??`Consulta los datos y apuesta con responsabilidad.`),fecha_generacion:String(e.fecha_generacion??``),fecha_evento:String(e.fecha_evento??``),horario:String(e.horario??``),estado:t.includes(n)?n:`revision_pendiente`,es_parlay:!!e.es_parlay,visibility:e.visibility===`premium`?`premium`:`public`}}function m(e){let t=e.find(e=>e.estado===`pendiente`&&!e.es_parlay);return t?[{...p(t),visibility:`public`}]:[]}async function h(){let e=await fetch(`/picks.json`,{cache:`no-store`});return e.ok?m(await e.json()):[]}var g={picks:[],history:[],pickFilter:`all`,historyFilter:`all`,user:null,isVip:!1};t(),i();var _=e=>document.getElementById(e);function v(e){let t=e.toLowerCase();return t.includes(`liga mx`)?`ligamx`:t.includes(`mlb`)?`mlb`:`futbol`}function y(e){if(!e)return`—`;let t=new Date(`${e}T12:00:00-06:00`);return Number.isNaN(t.getTime())?f(e):new Intl.DateTimeFormat(`es-MX`,{day:`2-digit`,month:`short`,timeZone:`America/Mexico_City`}).format(t)}function b(e){let t=e.visibility===`premium`&&!g.isVip;return`
    <article class="pick-card ${t?`locked`:``}">
      <div class="pick-meta"><span>${f(e.categoria)}</span><span>${y(e.fecha_evento||e.fecha_generacion)} · CDMX</span></div>
      <h3>${f(e.partido)}</h3>
      ${t?`<div class="locked-pick"><strong>♛ Selección VIP</strong><span>Inicia sesión con una membresía activa para verla.</span></div>`:`
        <div class="selection-row"><span>Selección</span><strong>${f(e.pick)}</strong><b>@ ${f(e.cuota)}</b></div>
        <p>${f(e.razonamiento)}</p>
      `}
      <div class="pick-footer"><span>Confianza: ${f(e.confianza)}</span><span class="status status-${e.estado}">${o(e.estado)}</span></div>
    </article>`}function x(){let e=_(`picks-container`);if(!e)return;let t=g.picks.filter(e=>g.pickFilter===`all`||v(e.categoria)===g.pickFilter);e.innerHTML=t.length?t.map(b).join(``):`
    <div class="state-card"><strong>No hay picks disponibles en este filtro.</strong><span>Vuelve más tarde; no publicamos selecciones solo para llenar espacio.</span></div>`;let n=_(`picks-updated`);n&&(n.textContent=g.isVip?`Cartera VIP activa`:t.length?`1 selección pública`:`Sin selección disponible`)}function S(){let e=_(`history-container`);if(!e)return;let t=n(g.history).filter(e=>g.historyFilter===`all`||e.estado===g.historyFilter);e.innerHTML=t.length?t.map(e=>`
    <tr><td>${y(e.fecha_evento||e.fecha_generacion)}</td><td>${f(e.partido)}</td><td>${f(e.pick)}</td><td>@ ${f(e.cuota)}</td><td><span class="status status-${e.estado}">${o(e.estado)}</span></td></tr>
  `).join(``):`<tr><td colspan="5">Todavía no hay resultados en este filtro.</td></tr>`;let r=a(g.history),i=_(`metric-record`),s=_(`metric-units`),c=_(`metric-roi`);i&&(i.textContent=`${r.wins}-${r.losses}`),s&&(s.textContent=`${r.units>=0?`+`:``}${r.units} u`),c&&(c.textContent=`${r.roi>=0?`+`:``}${r.roi}%`)}async function C(){g.picks=await h(),g.history=[],x(),S()}var w=_(`auth-dialog`),T=()=>w?.showModal();document.querySelectorAll(`[data-auth-mode]`).forEach(e=>{e.addEventListener(`click`,()=>{document.querySelectorAll(`[data-auth-mode]`).forEach(t=>t.classList.toggle(`active`,t===e));let t=e.dataset.authMode===`register`,n=_(`auth-submit`),r=_(`auth-password`);n&&(n.textContent=t?`Crear cuenta`:`Iniciar sesión`),r&&(r.autocomplete=t?`new-password`:`current-password`)})}),_(`login-button`)?.addEventListener(`click`,async()=>{T()}),_(`signout-button`)?.addEventListener(`click`,async()=>{w?.close()}),_(`telegram-link-button`)?.addEventListener(`click`,async()=>{_(`auth-message`)}),_(`promo-form`)?.addEventListener(`submit`,async e=>{e.preventDefault(),_(`auth-message`),_(`promo-code`)?.value.trim()}),_(`auth-form`)?.addEventListener(`submit`,async e=>{e.preventDefault(),_(`auth-email`)?.value.trim(),_(`auth-password`)?.value,document.querySelector(`[data-auth-mode].active`)?.dataset.authMode;let t=_(`auth-message`);t&&(t.textContent=`La cuenta requiere configurar Supabase en este despliegue.`)});async function E(){if(g.isVip,!g.user){T();let e=_(`auth-message`);e&&(e.textContent=`Crea una cuenta o inicia sesión antes de pagar.`);return}}_(`vip-button`)?.addEventListener(`click`,E),_(`vip-checkout-button`)?.addEventListener(`click`,E),_(`filter-row`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(g.pickFilter=t.dataset.filter??`all`,document.querySelectorAll(`[data-filter]`).forEach(e=>e.classList.toggle(`active`,e===t)),x())}),_(`history-filters`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-status]`);t&&(g.historyFilter=t.dataset.status??`all`,document.querySelectorAll(`[data-status]`).forEach(e=>e.classList.toggle(`active`,e===t)),S())});function D(){let e=Math.max(0,Number(_(`bankroll`)?.value||0)),t=Number(_(`risk-percent`)?.value||1),n=_(`stake-result`);n&&(n.textContent=`Unidad sugerida: $${Math.round(e*t/100).toLocaleString(`es-MX`)} MXN`)}_(`bankroll`)?.addEventListener(`input`,D),_(`risk-percent`)?.addEventListener(`change`,D),_(`telegram-cta`)?.addEventListener(`click`,()=>u(`telegram_clicked`)),d(document.querySelector(`.vip-section`),`vip_offer_viewed`);var O=_(`cookie-notice`),k=s(void 0,void 0),A=()=>c(_(`ad-slot-feed`),k);localStorage.getItem(`rey-taco-cookie-notice`)?A():O?.classList.remove(`hidden`),_(`cookie-accept`)?.addEventListener(`click`,()=>{localStorage.setItem(`rey-taco-cookie-notice`,`accepted`),O?.classList.add(`hidden`),A()}),(async()=>{await C()})();