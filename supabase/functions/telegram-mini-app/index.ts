...
const PICK_FIELDS =
  "partido,pick,cuota,categoria,estado,source_starts_at,fecha_evento,horario";
...
export async function verifyTelegramInitData(
  ...
  try {
    ...
    if (
      ...
      params.getAll("auth_date").length !== 1 || !/^\d+$/.test(authDate)
    ) invalidInitData();
    const authTimestamp = Number(authDate);
    if (
      !Number.isSafeInteger(authTimestamp) || authTimestamp <= 0 ||
      nowSeconds - authTimestamp > MAX_AGE_SECONDS ||
      authTimestamp - nowSeconds > MAX_FUTURE_SKEW_SECONDS
    ) invalidInitData();
    const secret = new Uint8Array(
      await signHmac(new TextEncoder().encode("WebAppData"), botToken),
    );
    const expected = hex(
      await signHmac(secret, buildTelegramDataCheckString(initData)),
    );
    ...
    if (
      !user || (typeof user.id !== "string" && typeof user.id !== "number") ||
      String(user.id).trim() === ""
    ) invalidInitData();
    return {
      id: String(user.id),
...
function mexicoDateKey(now: Date): string {
  const values = Object.fromEntries(
    new Intl.DateTimeFormat("en-CA", {
      ...
      month: "2-digit",
      day: "2-digit",
    }).formatToParts(now).filter((part) => part.type !== "literal").map((
      part,
...
function slotFromRow(row: Record<string, unknown>): number | null {
  const source = typeof row.source_starts_at === "string"
    ? new Date(row.source_starts_at)
    : null;
  ...
  return match
    ? Math.min(3, Math.floor(Number(String(row.horario).slice(0, 2)) / 6))
    : null;
}
...
function jsonResponse(
  ...
  return new Response(JSON.stringify(value), {
    status,
    headers: headers(request, siteUrl),
  });
}

function groupRows(rows: Record<string, unknown>[], date: string) {
  const windows = WINDOW_SLOTS.map((slot) => ({
    slot,
...
type MiniAppHandlerDependencies = Readonly<{
  createAdminClient?: (url: string, key: string) => unknown;
  env?: (name: string) => string | undefined;
  now?: () => Date;
  nowSeconds?: () => number;
}>;

type AdminResult = Readonly<{ data: unknown; error: unknown }>;
type AdminQuery = {
  select(fields: string): AdminQuery;
  eq(field: string, value: unknown): AdminQuery;
  in(field: string, values: unknown[]): AdminQuery;
  order(field: string, options: { ascending: boolean }): AdminQuery;
  limit(value: number): Promise<AdminResult>;
  maybeSingle(): Promise<AdminResult>;
};
type MiniAppAdmin = {
  from(table: string): AdminQuery;
  rpc(name: string, arguments_: Record<string, unknown>): Promise<AdminResult>;
};

export function createTelegramMiniAppHandler(
  dependencies: MiniAppHandlerDependencies = {},
  ...
  return async (request: Request): Promise<Response> => {
    const siteUrl = (env("SITE_URL") ?? "https://reytacopicks.com").replace(
      /\/$/,
      "",
    );
    const responseHeaders = headers(request, siteUrl);
    const origin = request.headers.get("Origin");
    if (origin && !publicOrigin(request, siteUrl)) {
    ...
    try {
      ...
      const date = mexicoDateKey(now());

      const profile = await admin.from("profiles").select("id").eq(
        "telegram_id",
        identity.id,
      ).maybeSingle();
      const profileRow = profile.data as { id?: string } | null;
      const linked = !profile.error && Boolean(profileRow?.id);
      let isVip = false;
      if (linked) {
        const membership = await admin.rpc("is_active_subscriber", {
          check_user: profileRow!.id,
        });
      ...
      const premiumRows = (premiumResult.data ?? []) as unknown as Record<
        string,
        unknown
      >[];
      const windows = groupRows(
      ...
      for (const slot of WINDOW_SLOTS) {
        ...
        if (isVip) {
          windows[slot].picks.push(
            ...premiumRows.filter((row) => slotFromRow(row) === slot).map(
              safePick,
            ),
          );
        }
      ...
      return jsonResponse(request, siteUrl, {
        date,
        windows,
        history: ((historyResult.data ?? []) as Record<string, unknown>[]).map(
          safePick,
        ),
        linked,
        is_vip: isVip,
      ...
      return new Response("Unable to load mini app", {
        status: 400,
        headers: responseHeaders,
      });
    }
...
const handle = createTelegramMiniAppHandler();
